        ...
        return (
            <AdminLayout onLogout={handleLogout} title="Bookie Detail">
                <div className="flex flex-col items-center justify-center min-h-[40vh]">
                    ...
                    <Link to="/revenue" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary-500 text-white font-semibold">
    ...
    return (
        <AdminLayout onLogout={handleLogout} title="Bookie Detail">
            <div className="min-w-0 max-w-full space-y-4 sm:space-y-6">
                {/* Back + Header */}
                <div>
                    <Link to="/revenue" className="text-gray-400 hover:text-primary-400 text-sm inline-flex items-center gap-1 mb-2">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${bookie?.status === 'active' ? 'bg-primary-500/20 text-emerald-600' : 'bg-red-500/200/20 text-red-500'}`}>
                            {bookie?.status === 'active' ? 'Active' : 'Inactive'}
                        </span>
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary-500/15 text-primary-400">
                            {bookie?.commissionPercentage || 0}% Commission
                        </span>
                    </div>
                    {bookie?.phone && <p className="text-gray-400 text-sm mt-0.5">{bookie.phone}</p>}
                </div>
                        ...
                        {PRESETS.map((p) => (
                            <button
                                ...
                                className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-colors ${
                                    activePreset === p.id ? 'bg-primary-500 text-white' : 'bg-[#1F2732] text-gray-300 hover:bg-[#252D3A]'
                                }`}
                            >
                                {p.label}
                            </button>
                ...
                {/* Tabs */}
                <div className="flex flex-wrap gap-1.5 sm:gap-2 border-b border-[#333D4D] pb-2">
                    {TABS.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-semibold text-xs sm:text-sm transition-colors ${
                                activeTab === tab.id ? 'bg-primary-500 text-white' : 'bg-[#1F2732] text-gray-300 hover:bg-[#252D3A]'
                            }`}
                        >
                            {tab.label}
                        </button>
                ...
                {activeTab === 'overview' && revenue && (
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-[#252D3A] rounded-xl p-3 sm:p-4 border border-[#333D4D]/60 relative overflow-hidden">
                                ...
                                <p className="text-[10px] text-gray-400 mt-0.5">{formatNumber(revenue.totalBetCount)} bets</p>
                            </div>
                            <div className="bg-[#252D3A] rounded-xl p-3 sm:p-4 border border-[#333D4D]/60 relative overflow-hidden">
                                <div className="absolute top-0 left-0 right-0 h-1 bg-red-500/200" />
                                ...
                                <p className="text-[10px] text-gray-400 mt-0.5">{formatNumber(revenue.winningBets)} wins, {formatNumber(revenue.losingBets)} losses</p>
                            </div>
                            <div className="bg-[#252D3A] rounded-xl p-3 sm:p-4 border border-[#333D4D]/60 relative overflow-hidden">
                            ...
                            <div className={`bg-[#252D3A] rounded-xl p-3 sm:p-4 border relative overflow-hidden ${revenue.adminProfit >= 0 ? 'border-emerald-500/40' : 'border-red-500/40'}`}>
                                <div className={`absolute top-0 left-0 right-0 h-1 ${revenue.adminProfit >= 0 ? 'bg-primary-500' : 'bg-red-500/200'}`} />
                        ...
                        {/* Breakdown */}
                        <div className="bg-[#252D3A] rounded-xl border border-[#333D4D]/60 p-3 sm:p-5">
                            <h3 className="text-sm font-semibold text-white mb-3">Revenue Breakdown</h3>
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between py-1.5 border-b border-[#333D4D]/40">
                                ...
                                <div className="flex justify-between py-1.5 border-b border-[#333D4D]/40">
                                    ...
                                    <span className="text-white font-medium">{formatNumber(revenue.totalBetCount)} bets <span className="text-gray-400">({formatNumber(revenue.winningBets)} W / {formatNumber(revenue.losingBets)} L)</span></span>
                                </div>
                                <div className="flex justify-between py-1.5 border-b border-[#333D4D]/40">
                                ...
                                <div className="flex justify-between py-2 font-bold">
                                    <span className="text-primary-400">Admin Profit</span>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-[#252D3A] rounded-lg p-3 text-center border border-[#333D4D]/40">
                                <p className="text-[10px] sm:text-xs text-gray-400 uppercase">Total Users</p>
                                <p className="text-lg sm:text-xl font-bold text-white">{formatNumber(data?.totalUsers)}</p>
                            </div>
                            <div className="bg-[#252D3A] rounded-lg p-3 text-center border border-[#333D4D]/40">
                                <p className="text-[10px] sm:text-xs text-gray-400 uppercase">Win Rate</p>
                                <p className="text-lg sm:text-xl font-bold text-white">{revenue.winRate}%</p>
                            </div>
                            <div className="bg-[#252D3A] rounded-lg p-3 text-center border border-[#333D4D]/40">
                                <p className="text-[10px] sm:text-xs text-gray-400 uppercase">Total Bets</p>
                ...
                {activeTab === 'users' && (
                    <div className="bg-[#252D3A] rounded-xl border border-[#333D4D] overflow-hidden">
                        <div className="px-3 sm:px-5 py-3 border-b border-[#333D4D]">
                        ...
                        <div className="hidden md:block overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-700/40">
                                    {users.map((u) => (
                                        <tr key={u.userId} className="hover:bg-[#1F2732]/20 transition-colors">
                                            <td className="px-4 py-2.5">
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-2 h-2 rounded-full shrink-0 ${u.isActive ? 'bg-emerald-400' : 'bg-red-400'}`} />
                                                    <Link to={`/all-users/${u.userId}`} className="font-medium text-white hover:text-primary-400 transition-colors">{u.username}</Link>
                                                </div>
                                                {u.phone && <p className="text-[11px] text-gray-400 ml-4">{u.phone}</p>}
                                            </td>
                                ...
                                <div key={u.userId} className="p-3">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <div className={`w-2 h-2 rounded-full shrink-0 ${u.isActive ? 'bg-emerald-400' : 'bg-red-400'}`} />
                                            <Link to={`/all-users/${u.userId}`} className="font-medium text-white text-sm hover:text-primary-400 truncate">{u.username}</Link>
                                        </div>
                        ...
                        {users.length === 0 && (
                            <div className="p-8 text-center text-gray-400 text-sm">No users found for this bookie</div>
                        )}
                    </div>
                )}
                ...
                {activeTab === 'bets' && (
                    <div className="bg-[#252D3A] rounded-xl border border-[#333D4D] overflow-hidden">
                        ...
                        <div className="divide-y divide-gray-700/40">
                            {bets.map((b) => (
                                <div key={b._id} className="p-3 sm:p-4 hover:bg-[#1F2732]/20 transition-colors">
                                    <div className="flex flex-wrap items-center justify-between gap-2 mb-1.5">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <span className="text-primary-400 font-mono font-medium text-sm">{b.betNumber}</span>
                                            <span className="text-gray-400 text-xs">{b.betType}</span>
                                            {b.betOn && <span className="text-gray-300 text-[10px] uppercase">({b.betOn})</span>}
                                        </div>
                                        <span className={`px-2 py-0.5 rounded text-[11px] font-semibold ${
                                            b.status === 'won' ? 'bg-emerald-900/50 text-emerald-600' :
                                            b.status === 'lost' ? 'bg-red-500/20 text-red-500' :
                                            'bg-[#252D3A] text-gray-300'
                                        }`}>{b.status}</span>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
                                        <span><span className="text-gray-400">Player:</span> <span className="text-white">{b.username}</span></span>
                                        <span><span className="text-gray-400">Market:</span> <span className="text-gray-300">{b.marketName}</span></span>
                                        <span><span className="text-gray-400">Amt:</span> <span className="text-white font-mono">{formatCurrency(b.amount)}</span></span>
                                        {b.payout > 0 && (
                                            <span><span className="text-gray-400">Pay:</span> <span className="text-emerald-600 font-mono">{formatCurrency(b.payout)}</span></span>
                                        )}
                                        <span className="text-gray-300">{new Date(b.createdAt).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        {bets.length === 0 && (
                            <div className="p-8 text-center text-gray-400 text-sm">No bets found for this period</div>
                        )}
                    </div>
                )}

                {activeTab === 'profile' && bookie && (
                    <div className="bg-[#252D3A] rounded-xl border border-[#333D4D] p-4 sm:p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg bg-primary-500/15 flex items-center justify-center shrink-0">
                                    <FaUserTie className="w-4 h-4 text-primary-400" />
                                </div>
                            ...
                            <div className="flex items-center gap-3">
                                <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${bookie.status === 'active' ? 'bg-primary-500/15' : 'bg-red-500/200/15'}`}>
