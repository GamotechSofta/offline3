    ...
    return (
        <Layout title={t('shortcuts')}>
            <div className="max-w-4xl mx-auto p-4 sm:p-6">
                <div className="bg-[#252D3A] rounded-xl border-2 border-[#333D4D] shadow-lg overflow-hidden">
                    {/* Header */}
                    <div className="bg-gradient-to-r from-primary-500 to-primary-600 p-6 text-white">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-primary-100 text-sm mt-1">Speed up your workflow with these shortcuts</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-6 space-y-8">
                        {shortcuts.map((category, catIdx) => (
                            <div key={catIdx} className="border-b border-[#333D4D] last:border-b-0 pb-6 last:pb-0">
                                ...
                                <div className="space-y-3">
                                    {category.items.map((item, itemIdx) => (
                                        <div
                                            key={itemIdx}
                                            className="flex items-start gap-4 p-4 rounded-lg bg-[#1F2732] hover:bg-primary-500/20 transition-colors border border-[#333D4D]"
                                        >
                                            {item.icon && (
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-gray-300 text-sm sm:text-base mb-2">{item.description}</p>
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    {item.keys.map((key, keyIdx) => (
                                                        <React.Fragment key={keyIdx}>
                                                            <kbd className="px-3 py-1.5 bg-[#252D3A] border border-[#333D4D] rounded-lg text-sm font-mono font-semibold text-gray-300 shadow-sm">
                                                                {formatKey(key)}
                                                            </kbd>
                    ...
                    {/* Footer Note */}
                    <div className="bg-[#1F2732] border-t border-[#333D4D] p-4">
