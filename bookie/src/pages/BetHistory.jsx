...
const BetHistory = () => {
    ...
    return (
        <Layout title={t('betHistory')}>
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-6">{t('betHistoryTitle')}</h1>
            <div className="bg-[#252D3A] rounded-lg p-4 mb-4 sm:mb-6 flex flex-wrap gap-3 items-center border border-[#333D4D]">
                <input type="text" placeholder={t('playerId')} value={filters.userId} onChange={(e) => setFilters({ ...filters, userId: e.target.value })} className="px-4 py-2 bg-[#1F2732] border border-[#333D4D] rounded-lg text-white" />
                <input type="text" placeholder={t('marketId')} value={filters.marketId} onChange={(e) => setFilters({ ...filters, marketId: e.target.value })} className="px-4 py-2 bg-[#1F2732] border border-[#333D4D] rounded-lg text-white" />
                ...
                <input type="date" value={filters.startDate} onChange={(e) => setFilters({ ...filters, startDate: e.target.value })} className="px-4 py-2 bg-[#1F2732] border border-[#333D4D] rounded-lg text-white" />
                <input type="date" value={filters.endDate} onChange={(e) => setFilters({ ...filters, endDate: e.target.value })} className="px-4 py-2 bg-[#1F2732] border border-[#333D4D] rounded-lg text-white" />
            </div>
            {loading ? (
                    ...
                    <table className="w-full">
                        <thead className="bg-[#1F2732]">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">{t('market')}</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">{t('betType')}</th>
                            ...
                            {bets.length === 0 ? (
                                ...
                                bets.map((bet) => (
                                    <tr key={bet._id} className="hover:bg-[#1F2732]">
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                            <span className={`px-2 py-1 rounded text-xs ${bet.status === 'won' ? 'bg-green-600' : bet.status === 'lost' ? 'bg-red-600' : bet.status === 'pending' ? 'bg-primary-600' : 'bg-[#333D4D]'}`}>{bet.status}</span>
                                        </td>
