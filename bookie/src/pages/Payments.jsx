    ...
    return (
        <Layout title="Payments">
            ...
            <div className="bg-gray-800 rounded-lg p-4 mb-6 flex gap-4">
                ...
                <select value={filters.type} onChange={(e) => setFilters({ ...filters, type: e.target.value })} className="px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white">
                            ...
                            {payments.length === 0 ? (
                                ...
                                payments.map((p) => (
                                    <tr key={p._id} className="hover:bg-gray-700">
                                        ...
                                        <td className="px-6 py-4 text-sm"><span className={`px-2 py-1 rounded text-xs ${p.status === 'approved' || p.status === 'completed' ? 'bg-green-600' : p.status === 'pending' ? 'bg-yellow-600' : 'bg-red-600'}`}>{p.status}</span></td>
