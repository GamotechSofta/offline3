...
const WithdrawFund = () => {
    ...
    return (
        <div className="space-y-6">
            {/* Wallet Balance Card */}
            <div className="rounded-2xl bg-[#252D3A] p-0 border-2 border-[#333D4D]">
                <div className="bg-[#252D3A] rounded-2xl shadow-sm border-2 border-[#333D4D] overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-primary-500 via-primary-500 to-primary-600 px-4 py-3 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-white/20 border border-white/30 flex items-center justify-center shrink-0">
                            <div className="w-8 h-8 rounded-full bg-[#252D3A] flex items-center justify-center text-sm font-extrabold text-primary-400">
                                ₹
                            </div>
                        </div>
                    ...
                    <div className="px-4 py-3 flex items-center justify-between bg-primary-500/20 border-t border-[#333D4D]">
                        <div className="text-sm text-white truncate">
                            {user?.username || user?.name || 'User'}
                        </div>
                        <div className="flex items-center gap-1.5">
                            <span className="w-3 h-3 rounded-full bg-primary-500 inline-block" />
                            <span className="w-3 h-3 rounded-full bg-primary-400 inline-block" />
                        </div>
                    </div>
                </div>
            ...
            {/* Messages */}
            {error && (
                <div className="p-4 bg-red-500/20 border border-red-400/50 rounded-xl text-red-400 text-sm">
                    {error}
                </div>
            )}
            {success && (
                <div className="p-4 bg-green-500/20 border border-green-400/50 rounded-xl text-green-400 text-sm">
                    {success}
                </div>
            )}
            ...
            {bankAccounts.length === 0 && (
                <div className="p-3 bg-primary-500/20 border border-primary-400/50 rounded-xl text-primary-400 text-xs sm:text-sm">
                    <p className="font-medium">No bank account added!</p>
                    <p className="text-primary-300 mt-1 leading-snug">Please add a bank account first from the "Bank Detail" section to withdraw funds.</p>
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                    ...
                    <input
                        ...
                        className="w-full bg-[#1F2732] border-2 border-[#333D4D] rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {bankAccounts.map((acc) => (
                            <label
                                key={acc._id}
                                className={`flex items-center p-4 bg-[#252D3A] border-2 rounded-xl cursor-pointer transition-colors ${
                                    selectedBankId === acc._id
                                        ? 'border-primary-500 bg-primary-500/20'
                                        : 'border-[#333D4D] hover:border-primary-400/50'
                                }`}
                            >
                                <input
                                    type="radio"
                                ...
                                {acc.isDefault && (
                                    <span className="px-2 py-1 bg-primary-500/20 text-primary-400 text-xs rounded-full border border-primary-400/50">
                                        Default
                                    </span>
                                )}
                                <div className={`w-5 h-5 rounded-full border-2 ml-3 flex items-center justify-center ${
                                    selectedBankId === acc._id ? 'border-primary-500' : 'border-[#333D4D]'
                                }`}>
                                    {selectedBankId === acc._id && (
                ...
                <div>
                    ...
                    <textarea
                        ...
                        className="w-full bg-[#1F2732] border-2 border-[#333D4D] rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 resize-none"
                        rows={2}
                    />
                </div>
                ...
                <button
                    ...
                    className="w-full py-4 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white font-bold rounded-xl transition-all disabled:opacity-50 shadow-md hover:shadow-lg"
                >
            ...
            <div className="bg-[#252D3A] rounded-xl p-4 border-2 border-[#333D4D]">
                <h4 className="text-primary-400 font-semibold mb-2">Withdrawal Info:</h4>
            ...
            {showSuccessModal && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                    <div className="bg-[#252D3A] rounded-2xl max-w-sm w-full p-6 border-2 border-[#333D4D] shadow-xl text-center">
                        {/* Success Icon */}
                        ...
                        <div className="bg-primary-500/20 rounded-xl p-4 mb-4 border-2 border-[#333D4D]">
                            <p className="text-gray-400 text-sm">Amount</p>
                            <p className="text-2xl font-bold text-primary-400">₹{submittedAmount.toLocaleString()}</p>
                        </div>
                        ...
                        <div className="space-y-3">
                            <button
                                ...
                                className="w-full py-3 bg-primary-500 hover:bg-primary-600 text-white font-semibold rounded-xl transition-colors shadow-md"
                            >
                                Done
                            </button>
                            <button
                                ...
                                className="w-full py-3 bg-[#252D3A] border-2 border-[#333D4D] hover:border-primary-400 hover:bg-primary-500/20 text-white font-medium rounded-xl transition-colors"
                            >
                                View History
                            </button>
                        </div>
