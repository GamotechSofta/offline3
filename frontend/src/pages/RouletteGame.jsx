...
// Black numbers that use the custom SVG as background (right-side number grid)
...
const RouletteGame = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      setError('');
      try {
        const [balRes, statsRes, histRes] = await Promise.all([
          getBalance(),
          fetch(`${API_BASE_URL}/roulette/stats?userId=${encodeURIComponent(userId)}`).then((r) => r.json()),
          fetch(`${API_BASE_URL}/roulette/history?userId=${encodeURIComponent(userId)}&limit=10`).then((r) => r.json()),
        ]);
        ...
        <div className="mb-3 md:mb-6 grid grid-cols-1 lg:grid-cols-[minmax(0,420px)_1fr] gap-3 md:gap-6">
          ...
          <div className="rounded-2xl bg-transparent md:bg-[#186213]/40 border-0 md:border md:border-yellow-500/60 p-2 md:p-4 flex flex-col max-h-[640px] overflow-y-auto min-w-0">
            ...
            <div className="flex-shrink-0">
              ...
              {/* Smaller zero chip above the grid */}
              <div className="mb-1">
                <button
                  type="button"
              ...
              {/* 3 rows × 12 cols (36 numbers) - constrain width to prevent overlap */}
            ...
            {/* Bet amount, quick bets and current bets - below numbers with clear gap */}
