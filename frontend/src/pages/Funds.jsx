...
const Funds = () => {
  ...
  /* Body scroll lock removed: was locking entire page on mobile and broke inner scroll.
   * Lock body only when a modal is open; inner overflow-y-auto areas scroll via global CSS. */
  ...
  return (
    <div className="min-h-screen bg-[#1F2732] text-white pl-3 pr-3 sm:pl-4 sm:pr-4 pt-0 md:pt-4 pb-[calc(6rem+env(safe-area-inset-bottom,0px))]">
      <div className="w-full max-w-lg md:max-w-none mx-auto md:mx-0">
        <div className="mb-4 md:grid md:grid-cols-[360px_1fr] md:gap-6 md:items-center">
          <div className="flex items-center gap-3 pt-4 md:pt-0">
            <button
              ...
              className="min-w-[44px] min-h-[44px] rounded-full bg-[#252D3A] border border-[#333D4D] flex items-center justify-center text-white hover:bg-primary-500/20 active:scale-95 transition touch-manipulation"
              aria-label="Back"
            >
        ...
        <div className="md:hidden">
          {mobileView === null ? (
            ...
            <div className="space-y-2.5">
              {items.map((item) => (
                <div
                  ...
                  className="bg-[#252D3A] border-2 border-[#333D4D] rounded-2xl p-3 flex items-center justify-between shadow-sm hover:border-primary-400 transition-colors"
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] sm:text-xs text-gray-300 leading-snug">{item.subtitle}</p>
                    </div>
                  </div>
                  <div className="w-8 h-8 rounded-full bg-primary-500/20 border border-primary-400 flex items-center justify-center text-primary-400">
            ...
            // Component view
            <div
              className={`bg-[#252D3A] border-2 border-[#333D4D] rounded-2xl shadow-sm ${
                isAddFundMobileView
        ...
        <div className="hidden md:grid md:grid-cols-[360px_1fr] md:gap-6 md:items-start">
            ...
            {items.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left bg-[#252D3A] border-2 rounded-2xl p-3 md:p-5 flex items-center justify-between shadow-sm transition-colors ${
                    active ? 'border-primary-500 bg-primary-500/20' : 'border-[#333D4D] hover:border-primary-400'
                  }`}
                >
                  ...
                  <div
                    className={`w-8 h-8 md:w-9 md:h-9 rounded-full border flex items-center justify-center ${
                      active ? 'bg-primary-500 border-primary-500 text-white' : 'bg-[#1F2732] border-[#333D4D] text-gray-400'
                    }`}
                  >
          ...
          <main className="rounded-2xl bg-[#252D3A] border-2 border-[#333D4D] shadow-sm p-6">
