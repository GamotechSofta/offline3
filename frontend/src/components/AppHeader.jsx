  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-50 w-full bg-black px-3 sm:px-5 md:px-6 lg:px-8 py-2.5 sm:py-3 md:py-4">
        ...
        >
          <h1 className="text-[var(--theme-primary)] text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold lowercase relative">
            logoipsum
            <span className="absolute bottom-0 left-0 w-10 sm:w-12 md:w-14 h-0.5 bg-[var(--theme-primary)]"></span>
            <span className="absolute bottom-0 right-0 w-8 sm:w-10 md:w-12 h-0.5 bg-[var(--theme-primary)]"></span>
            <svg className="absolute -top-0.5 sm:-top-1 right-1.5 sm:right-2 w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-3.5 md:h-3.5 text-[var(--theme-primary)]" fill="currentColor" viewBox="0 0 20 20">
        ...
        <button
          ...
          className="shrink-0 rounded-lg sm:rounded-xl md:rounded-xl bg-[var(--theme-primary)] px-2.5 sm:px-3 md:px-4 lg:px-5 py-1.5 sm:py-2 md:py-2.5 text-xs sm:text-sm md:text-base font-semibold text-black shadow-[0_4px_12px_rgba(0,0,0,0.25)] hover:opacity-90 transition-colors flex items-center gap-1 sm:gap-1.5 md:gap-2"
        >
        ...
        {/* Profile (replaces notification icon) */}
        <button
          type="button"
          ...
          className={`w-9 h-9 sm:w-10 sm:h-10 md:w-11 md:h-11 shrink-0 rounded-full bg-gray-800 border flex items-center justify-center cursor-pointer transition-colors ${
            user ? 'border-[var(--theme-primary)]/50 hover:bg-[var(--theme-primary)]/10' : 'border-gray-700 hover:bg-gray-700'
          }`}
          title={user ? `${displayName}` : 'Sign In / Sign Up'}
          aria-label="Profile"
        >
          <svg
            className={`w-4 h-4 sm:w-5 sm:h-5 md:w-5 md:h-5 ${
              user ? 'text-[var(--theme-primary)]' : 'text-white'
            }`}
          ...
          >
            {user ? (
              <path
                fillRule="evenodd"
                d="M12 12a4 4 0 100-8 4 4 0 000 8zm-9 10a9 9 0 0118 0H3z"
                clipRule="evenodd"
              />
            ) : (
              <path
